...
"[project]/app/components/ApiService.ts [app-client] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
class ApiService {
    ...
    async getListings() {
        try {
            const response = await fetch("".concat(this.baseUrl, "/listings"), {
                method: 'GET',
                headers: {
                    'Content-Type': 'application/json'
                }
    ...
    getMockListings() {
        return [
            {
                id: 'mock-1',
                item_name: 'Pizza Slices',
                photo_url: undefined,
                apartment_number: '101',
                building_number: 'Edge',
                days_to_expiry: 2,
                status: 'available',
                building_id: 'edge',
                user_id: 'mock-user-1',
            ...
            {
                id: 'mock-2',
                item_name: 'Energy Drinks',
                photo_url: undefined,
                apartment_number: '201',
                building_number: 'TechTerrace',
                days_to_expiry: 5,
                status: 'available',
                building_id: 'techterrace',
                user_id: 'mock-user-2',
                owner_name: 'Bob',
    ...
    // Get listings filtered by building
    async getListingsByBuilding(buildingId) {
        try {
            const allListings = await this.getListings();
            return allListings.filter((listing)=>listing.building_id === buildingId);
        } catch (error) {
            console.error('Error fetching listings by building:', error);
            throw error;
        }
    }
    ...
    async createListing(listingData) {
        try {
            ...
            if (!response.ok) {
                throw new Error("Failed to create listing: ".concat(response.statusText));
            }
    ...
    async createListingWithPhoto(photoFile, apartmentNumber, buildingNumber) {
        try {
            ...
            const response = await fetch("".concat(this.baseUrl, "/listings/photo"), {
                method: 'POST',
                headers,
                body: formData
            });
            if (!response.ok) {
                throw new Error("Failed to create listing with photo: ".concat(response.statusText));
            }
            const listing = await response.json();
            return listing;
        } catch (error) {
            console.error('Error creating listing with photo:', error);
            throw error;
        }
    }
    // Create listing with sample photo
    async createSampleListing(apartmentNumber, buildingNumber) {
        try {
            const requestBody = {};
            if (apartmentNumber) {
                requestBody.apartment_number = apartmentNumber;
            }
            if (buildingNumber) {
                requestBody.building_number = buildingNumber;
            }
            const response = await fetch("".concat(this.baseUrl, "/listings/photo/sample"), {
                method: 'POST',
                headers: this.getAuthHeaders(),
                body: JSON.stringify(requestBody)
            });
            if (!response.ok) {
                throw new Error("Failed to create sample listing: ".concat(response.statusText));
            }
            const result = await response.json();
            return result;
        } catch (error) {
            console.warn('Backend not available - simulating listing creation:', error);
            // Return mock result when backend is not available
            return {
                listing: {
                    id: "mock-new-".concat(Date.now()),
                    item_name: 'Sample Food Item',
                    photo_url: undefined,
                    apartment_number: apartmentNumber,
                    building_number: buildingNumber,
                    status: 'available',
                    building_id: buildingNumber || 'edge',
                    user_id: 'mock-user',
                    owner_name: 'You',
    ...
    async claimListing(listingId) {
        try {
            const response = await fetch("".concat(this.baseUrl, "/listings/").concat(listingId, "/claim"), {
                method: 'PUT',
                headers: this.getAuthHeaders()
            });
            if (!response.ok) {
                throw new Error("Failed to claim listing: ".concat(response.statusText));
            }
            console.log("✅ Successfully claimed listing ".concat(listingId));
        } catch (error) {
            console.warn('Backend not available - simulating claim:', error);
            // Simulate success when backend is not available
            console.log("✅ Simulated claim of listing ".concat(listingId));
        }
    }
    // Complete a listing (award points)
    async completeListing(listingId) {
        try {
            const response = await fetch("".concat(this.baseUrl, "/listings/").concat(listingId, "/complete"), {
                method: 'PUT',
                headers: this.getAuthHeaders()
            });
            if (!response.ok) {
                throw new Error("Failed to complete listing: ".concat(response.statusText));
            }
            console.log("🏆 Successfully completed listing ".concat(listingId));
        } catch (error) {
            console.warn('Backend not available - simulating completion:', error);
            // Simulate success when backend is not available
            console.log("🏆 Simulated completion of listing ".concat(listingId));
        }
    }
    // Mock authentication for demo purposes
    async mockLogin() {
        let buildingId = arguments.length > 0 && arguments[0] !== void 0 ? arguments[0] : 'edge';
        // In a real app, this would make an actual API call to authenticate
        // For now, we'll create a mock token
        const mockToken = "mock-token-".concat(buildingId, "-").concat(Date.now());
    ...
    constructor(){
        ...
        (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f2e$pnpm$2f40$swc$2b$helpers$40$0$2e$5$2e$15$2f$node_modules$2f40$swc$2f$helpers$2f$esm$2f$_define_property$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["_"])(this, "authToken", null);
...
(0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f2e$pnpm$2f40$swc$2b$helpers$40$0$2e$5$2e$15$2f$node_modules$2f40$swc$2f$helpers$2f$esm$2f$_define_property$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["_"])(ApiService, "instance", void 0);
...
var __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f2e$pnpm$2f40$swc$2b$helpers$40$0$2e$5$2e$15$2f$node_modules$2f40$swc$2f$helpers$2f$esm$2f$_define_property$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/node_modules/.pnpm/@swc+helpers@0.5.15/node_modules/@swc/helpers/esm/_define_property.js [app-client] (ecmascript)");
var __TURBOPACK__imported__module__$5b$project$5d2f$app$2f$components$2f$ApiService$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/app/components/ApiService.ts [app-client] (ecmascript)");
;
;
class FridgeManager {
    static getInstance() {
    ...
    async loadListingsFromDatabase(fridgeId) {
        try {
            this.isLoading = true;
            const buildingId = fridgeId; // Map fridge ID to building ID
            const listings = await this.apiService.getListingsByBuilding(buildingId);
            console.log("📦 Loaded ".concat(listings.length, " listings for ").concat(fridgeId, ":"), listings);
            const fridgeItems = listings.map((listing)=>({
                    id: "listing-".concat(listing.id),
                    listingId: listing.id,
                    name: listing.item_name,
                    quantity: 1,
                    addedBy: listing.owner_name || 'Unknown',
                    dateAdded: new Date(listing.created_at),
                    expirationDate: listing.days_to_expiry ? new Date(Date.now() + listing.days_to_expiry * 24 * 60 * 60 * 1000) : undefined,
                    category: this.categorizeItem(listing.item_name),
            ...
            return []; // Return empty array on error, keep fallback items
        } finally{
    ...
    categorizeItem(itemName) {
        ...
        if (name.includes('lettuce') || name.includes('carrot') || name.includes('vegetable') || name.includes('broccoli')) {
            return 'vegetables';
        }
        if (name.includes('apple') || name.includes('banana') || name.includes('orange') || name.includes('fruit')) {
            return 'fruits';
        }
        if (name.includes('pizza') || name.includes('leftover') || name.includes('sandwich') || name.includes('pasta')) {
            return 'leftovers';
        }
        if (name.includes('soda') || name.includes('juice') || name.includes('drink') || name.includes('water') || name.includes('energy')) {
            return 'beverages';
        }
        if (name.includes('sauce') || name.includes('ketchup') || name.includes('mustard') || name.includes('mayo')) {
            return 'condiments';
        }
        return 'other';
    }
    async refreshFridgeData(fridgeId) {
        ...
        if (now - lastRefresh > this.refreshInterval) {
            console.log("🔄 Refreshing data for fridge ".concat(fridgeId));
            const databaseItems = await this.loadListingsFromDatabase(fridgeId);
            const fridge = this.fridgeData.get(fridgeId);
            if (fridge) {
                // Keep manually added items (those without listingId) and merge with database items
                const manualItems = fridge.items.filter((item)=>!item.listingId);
                fridge.items = [
                    ...manualItems,
                    ...databaseItems
                ];
    ...
    async openFridgeUI(fridgeId) {
        ...
        // Force refresh when opening fridge
        this.lastRefresh.delete(fridgeId);
        await this.refreshFridgeData(fridgeId);
    }
    closeFridgeUI() {
    ...
    // UI Rendering
    async renderFridgeUI(ctx, playerId) {
        ...
        if (this.isLoading) {
            ...
            ctx.font = '12px Arial';
            ctx.textAlign = 'center';
            ctx.fillText('Loading...', uiX + uiWidth / 2, uiY + 420);
        }
        // Instructions
        ...
        ctx.textAlign = 'center';
        if (canModify) {
            ctx.fillText('A: Add local item | P: Add listing | C: Claim | X: Complete | D: Delete | ↑↓: Navigate | ESC: Close', uiX + uiWidth / 2, uiY + uiHeight - 20);
        } else {
            ctx.fillText('C: Claim | X: Complete | ↑↓: Navigate | ESC: Close', uiX + uiWidth / 2, uiY + uiHeight - 20);
        }
        ctx.restore();
    }
    ...
    renderFridgeItem(ctx, item, x, y, width, height, isSelected) {
        ...
        // Item background - different colors for different states
        if (isSelected) {
            ...
            ctx.fillStyle = '#EBF5FB'; // Light blue for database listings
        } else {
            ctx.fillStyle = '#FFFFFF'; // White default for manual items
        }
        ctx.fillRect(x, y, width, height);
        // Border - different colors for status
        ...
        // Status and metadata line
        ctx.fillStyle = isSelected ? '#ECF0F1' : '#7F8C8D';
        ctx.font = '9px Arial';
        if (isListing) {
            var _item_status;
            const statusText = item.status ? "[".concat((_item_status = item.status) === null || _item_status === void 0 ? void 0 : _item_status.toUpperCase(), "]") : '[AVAILABLE]';
            const ownerText = item.ownerName ? "by ".concat(item.ownerName) : "by ".concat(item.addedBy);
            ctx.fillText("".concat(statusText, " ").concat(ownerText), x + 30, y + 28);
            // Listing indicator
            ctx.fillStyle = isSelected ? '#FFFFFF' : '#3498DB';
            ctx.font = 'bold 8px Arial';
            ctx.fillText('📋 LISTING', x + width - 60, y + 15);
        } else {
    ...
    async selectNextItem() {
        ...
        const items = await this.getFridgeItems(this.uiState.currentFridge);
        if (items.length > 0) {
    ...
    async deleteSelectedItem(playerId) {
        ...
        if (items.length > 0 && this.uiState.selectedItemIndex < items.length) {
            ...
            // Only allow deletion of manually added items (non-listings)
            if (item.listingId) {
                console.log('Cannot delete database listings - use claim/complete instead');
                return false;
            }
    ...
    async claimSelectedItem(playerId) {
        if (!this.uiState.currentFridge) return false;
        const items = await this.getFridgeItems(this.uiState.currentFridge);
        if (items.length > 0 && this.uiState.selectedItemIndex < items.length) {
            const item = items[this.uiState.selectedItemIndex];
            if (item.listingId && item.status === 'available') {
                try {
                    await this.apiService.claimListing(item.listingId);
                    console.log("✅ Claimed listing: ".concat(item.name));
                ...
                console.log('Item cannot be claimed - either not a listing or already claimed');
                return false;
            }
        }
        return false;
    }
    async completeSelectedItem(playerId) {
        if (!this.uiState.currentFridge) return false;
        const items = await this.getFridgeItems(this.uiState.currentFridge);
        if (items.length > 0 && this.uiState.selectedItemIndex < items.length) {
            const item = items[this.uiState.selectedItemIndex];
            if (item.listingId && item.status === 'claimed') {
                try {
                    await this.apiService.completeListing(item.listingId);
                    console.log("🏆 Completed listing: ".concat(item.name, " - Points awarded!"));
                    // Force refresh to get updated status
                    this.lastRefresh.delete(this.uiState.currentFridge);
                    await this.refreshFridgeData(this.uiState.currentFridge);
                    return true;
                ...
                console.log('Item cannot be completed - either not a listing or not claimed');
                return false;
            }
        }
        return false;
    }
    async addSampleListing(fridgeId, playerId) {
        if (!this.canPlayerModify(fridgeId, playerId)) {
            console.log("Player ".concat(playerId, " cannot modify fridge ").concat(fridgeId, " - not a resident"));
            return false;
        }
        try {
            const fridge = this.fridgeData.get(fridgeId);
            const apartmentNumber = (fridge === null || fridge === void 0 ? void 0 : fridge.apartmentId) || undefined;
            const result = await this.apiService.createSampleListing(apartmentNumber, fridgeId);
            console.log("📸 Created sample listing:", result);
            // Force refresh to show new listing
            this.lastRefresh.delete(fridgeId);
            await this.refreshFridgeData(fridgeId);
            return true;
        } catch (error) {
            console.error('Failed to create sample listing:', error);
            return false;
        }
    }
    constructor(){
        ...
        (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f2e$pnpm$2f40$swc$2b$helpers$40$0$2e$5$2e$15$2f$node_modules$2f40$swc$2f$helpers$2f$esm$2f$_define_property$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["_"])(this, "apiService", void 0);
        ...
        (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f2e$pnpm$2f40$swc$2b$helpers$40$0$2e$5$2e$15$2f$node_modules$2f40$swc$2f$helpers$2f$esm$2f$_define_property$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["_"])(this, "lastRefresh", new Map());
        ...
        this.apiService = __TURBOPACK__imported__module__$5b$project$5d2f$app$2f$components$2f$ApiService$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["default"].getInstance();
        // Mock login for demo - in real app this would be handled by auth system
        this.apiService.mockLogin('edge');
    }
}
...
class Player {
    ...
    updateFridgeInteraction(inputHandler, scene) {
        ...
        if (fridgeManager.isFridgeUIOpen()) {
            // Don't await this to avoid blocking the game loop
            this.handleFridgeUIInput(inputHandler, fridgeManager).catch(console.error);
        }
    }
    async handleFridgeUIInput(inputHandler, fridgeManager) {
        ...
        if (inputHandler.wasKeyJustPressed('ArrowUp') || inputHandler.wasKeyJustPressed('KeyW')) {
            await fridgeManager.selectPreviousItem();
        }
        if (inputHandler.wasKeyJustPressed('ArrowDown') || inputHandler.wasKeyJustPressed('KeyS')) {
            await fridgeManager.selectNextItem();
        }
        // Claim listing with 'C' key (available to all players)
        if (inputHandler.wasKeyJustPressed('KeyC')) {
            console.log('🔑 C key pressed - attempting to claim selected item');
            await fridgeManager.claimSelectedItem(this.playerId);
        }
        // Complete listing with 'X' key (available to all players)
        if (inputHandler.wasKeyJustPressed('KeyX')) {
            console.log('🔑 X key pressed - attempting to complete selected item');
            await fridgeManager.completeSelectedItem(this.playerId);
        }
        if (canModify) {
            // Add local item with 'A' key
            if (inputHandler.wasKeyJustPressed('KeyA')) {
                fridgeManager.addRandomItem(currentFridge, this.playerId);
            }
            // Add sample listing with 'P' key  
            if (inputHandler.wasKeyJustPressed('KeyP')) {
                console.log('🔑 P key pressed - creating sample listing');
                await fridgeManager.addSampleListing(currentFridge, this.playerId);
            }
            // Delete selected item with 'D' key (only for manual items)
            if (inputHandler.wasKeyJustPressed('KeyD')) {
                await fridgeManager.deleteSelectedItem(this.playerId);
            }
        }
    }
...
class GameEngine {
    ...
    render() {
        ...
        const fridgeManager = __TURBOPACK__imported__module__$5b$project$5d2f$app$2f$components$2f$FridgeManager$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["default"].getInstance();
        // Don't await this to avoid blocking the render loop
        fridgeManager.renderFridgeUI(this.ctx, 'player1').catch(console.error); // Use actual player ID
    }
...
//# sourceMappingURL=app_8a72ea2e._.js.map