...
class ApiService {
    ...
    async completeItem(itemId) {
        return this.makeRequest("/listings/".concat(itemId, "/complete"), {
            method: 'PUT'
        });
    }
    // Health check
...
class FridgeManager {
    ...
    renderFridgeUI(ctx, playerId) {
        ...
        if (this.uiState.isInputMode) {
            ...
            const instructions = isOnline ? 'Press A to add item, D to delete selected item, C to claim, X to complete, R to refresh, ↑↓ to navigate, ESC to close' : 'Press A to add local item, D to delete item, R to retry connection, ↑↓ to navigate, ESC to close';
            ctx.fillText(instructions, uiX + uiWidth / 2, uiY + uiHeight - 10);
        } else {
            const instructions = isOnline ? 'Press C to claim items, X to complete, R to refresh, ESC to close' : 'Press R to retry connection, ESC to close';
            ctx.fillText(instructions, uiX + uiWidth / 2, uiY + uiHeight - 10);
        }
        ctx.restore();
    }
    ...
    async completeItem(itemId) {
        try {
            const response = await __TURBOPACK__imported__module__$5b$project$5d2f$app$2f$services$2f$ApiService$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["apiService"].completeItem(itemId);
            if (response.error) {
...
//# sourceMappingURL=app_4bd4adc9._.js.map