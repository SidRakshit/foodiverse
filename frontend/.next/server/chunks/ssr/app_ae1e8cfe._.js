module.exports = [
"[project]/app/components/ApiService.ts [app-ssr] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
class ApiService {
    ...
    async createListingWithPhoto(photoFile, apartmentNumber, buildingNumber) {
        try {
            ...
            const response = await fetch(`${this.baseUrl}/listings/photo`, {
                method: 'POST',
                headers,
                body: formData
            });
            if (!response.ok) {
                throw new Error(`Failed to create listing with photo: ${response.statusText}`);
            }
            const listing = await response.json();
            return listing;
        } catch (error) {
    ...
    async createSampleListing(apartmentNumber, buildingNumber) {
        try {
            ...
            const response = await fetch(`${this.baseUrl}/listings/photo/sample`, {
                method: 'POST',
                headers: this.getAuthHeaders(),
                body: JSON.stringify(requestBody)
            });
            if (!response.ok) {
                throw new Error(`Failed to create sample listing: ${response.statusText}`);
            }
    ...
    async claimListing(listingId) {
        try {
            const response = await fetch(`${this.baseUrl}/listings/${listingId}/claim`, {
                method: 'PUT',
                headers: this.getAuthHeaders()
            });
            if (!response.ok) {
                throw new Error(`Failed to claim listing: ${response.statusText}`);
            }
            console.log(`✅ Successfully claimed listing ${listingId}`);
        } catch (error) {
            console.warn('Backend not available - simulating claim:', error);
            // Simulate success when backend is not available
            console.log(`✅ Simulated claim of listing ${listingId}`);
        }
    }
    // Complete a listing (award points)
    async completeListing(listingId) {
        try {
            const response = await fetch(`${this.baseUrl}/listings/${listingId}/complete`, {
                method: 'PUT',
                headers: this.getAuthHeaders()
            });
            if (!response.ok) {
                throw new Error(`Failed to complete listing: ${response.statusText}`);
            }
            console.log(`🏆 Successfully completed listing ${listingId}`);
        } catch (error) {
            console.warn('Backend not available - simulating completion:', error);
            // Simulate success when backend is not available
            console.log(`🏆 Simulated completion of listing ${listingId}`);
        }
    }
    // Mock authentication for demo purposes
    async mockLogin(buildingId = 'edge') {
        // In a real app, this would make an actual API call to authenticate
        // For now, we'll create a mock token
        const mockToken = `mock-token-${buildingId}-${Date.now()}`;
...
var __TURBOPACK__imported__module__$5b$project$5d2f$app$2f$components$2f$ApiService$2e$ts__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/app/components/ApiService.ts [app-ssr] (ecmascript)");
;
class FridgeManager {
    static instance;
    fridgeData = new Map();
    ...
    constructor(){
        this.apiService = __TURBOPACK__imported__module__$5b$project$5d2f$app$2f$components$2f$ApiService$2e$ts__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["default"].getInstance();
    ...
    async loadListingsFromDatabase(fridgeId) {
        try {
            ...
            console.log(`📦 Loaded ${listings.length} listings for ${fridgeId}:`, listings);
    ...
    async refreshFridgeData(fridgeId) {
        ...
        if (now - lastRefresh > this.refreshInterval) {
            console.log(`🔄 Refreshing data for fridge ${fridgeId}`);
    ...
    renderFridgeItem(ctx, item, x, y, width, height, isSelected) {
        ...
        ctx.font = '9px Arial';
        if (isListing) {
            const statusText = item.status ? `[${item.status?.toUpperCase()}]` : '[AVAILABLE]';
            const ownerText = item.ownerName ? `by ${item.ownerName}` : `by ${item.addedBy}`;
            ctx.fillText(`${statusText} ${ownerText}`, x + 30, y + 28);
            // Listing indicator
        ...
        if (items.length > 0 && this.uiState.selectedItemIndex < items.length) {
            ...
            if (item.listingId && item.status === 'available') {
                try {
                    ...
                    console.log(`✅ Claimed listing: ${item.name}`);
        ...
        if (items.length > 0 && this.uiState.selectedItemIndex < items.length) {
            ...
            if (item.listingId && item.status === 'claimed') {
                try {
                    ...
                    console.log(`🏆 Completed listing: ${item.name} - Points awarded!`);
                    // Force refresh to get updated status
                    this.lastRefresh.delete(this.uiState.currentFridge);
                    await this.refreshFridgeData(this.uiState.currentFridge);
                    return true;
    ...
    async addSampleListing(fridgeId, playerId) {
        ...
        try {
            const fridge = this.fridgeData.get(fridgeId);
            const apartmentNumber = fridge?.apartmentId || undefined;
...
//# sourceMappingURL=app_ae1e8cfe._.js.map